...
function CreateEvent() {
  ...
  const [formData, setFormData] = useState({
    //store form data including event details and image URLs
    imageUrls: [],
    type: "",
    title: "",
    date: "",
  ...
  const handleSubmit = async (e) => {
    ...
    if (isNaN(formData.price)) {
      //If price is not a number, it displays an error message using Swal.fire.
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    if (formData.price <= 0) {
      //price must be greater than or equal to 0. if not display error msj
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    if (isNaN(formData.MaxParticipants)) {
      //participant must be a number. if not display error msj
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    if (formData.MaxParticipants < 0) {
      //participants must be greater than to 0.
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    if (selectedDate < currentDate) {
      //if selected date is before current date then display error msj
      Swal.fire({
        icon: "error",
        title: "Error",
    ...
    try {
      ...
      if (data.success === false) {
        //id data added successfully then display success msj
